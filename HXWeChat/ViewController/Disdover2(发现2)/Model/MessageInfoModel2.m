...
-(instancetype)initWithDic:(NSDictionary *)dic{
    ...
    if (self) {
        ...
        if (likeUsers.count) {
            ...
            model2.rowHeight = [mutablAttrStr.string boundingRectWithSize:CGSizeMake(HXWIDTH-kGAP-kAvatar_Size-2*kGAP, CGFLOAT_MAX) font:font lineSpacing:3.0].height+0.5+8+5;
        ...
        if ([attrString.string isMoreThanOneLineWithSize:CGSizeMake(HXWIDTH-kGAP-kAvatar_Size-2*kGAP, CGFLOAT_MAX) font:font lineSpaceing:3.0]) {//margin
        ...
        //算text的layout
        CGFloat textHeight = [attrString.string boundingRectWithSize:CGSizeMake(HXWIDTH-kGAP-kAvatar_Size-2*kGAP, CGFLOAT_MAX) font:font lineSpacing:3.0].height+0.5;
        
        self.textLayout.frameLayout = CGRectMake(kGAP+kAvatar_Size+kGAP, kGAP+kAvatar_Size/2+2, HXWIDTH-2*kGAP-kAvatar_Size-kGAP, textHeight);
        // 老大哥
        //算九宫格的layout
        ...
        // 加上评论的 高度
        CGFloat conmentH = 3+ 22 +3;
        self.headerHeight = CGRectGetMaxY(self.jggLayout.frameLayout)+kGAP+conmentH;    }
    return self;
}
